...
public class Person extends Point {
    ...
    public boolean limtMove(){
        return MathUtil.stdGaussian(sig,Constants.u-1.79f) > 0;
    }
    ...
    private void action() {
        ...
        if (lim == false && !wantMove()) {
            return;
        }else if (lim == true && !limtMove()){
            return;
        }
    ...
    private float SAFE_DIST = 1.5f;//安全距离
    ...
    public void update() {
        ...
        //处理治愈
        if (state == State.FREEZE && dieMoment == -1){
            int cureTime = (int) MathUtil.stdGaussian(1,Constants.CURE_TIME);
        ...
        //处理治愈患者
        if (state == State.FREEZE && MyPanel.worldTime >= cureMoment && cureMoment > 0){
            state = State.CURED;
        ...
        for (Person person : people) {
            ...
            if (wearmask ) {
                if (random < Constants.BROAD_RATE-0.5 && distance(person) < SAFE_DIST) {
                    this.beInfected();
                    break;
                }
            }else if (!wearmask  ){
